...
import { lingkaranQuizQuestions } from "@/data/lingkaranQuizData";
...
const LingkaranQuizPage = () => {
  ...
  const setupMeteors = useCallback((qIndex: number) => {
    const q = lingkaranQuizQuestions[qIndex];
        ...
        const step = () => {
          ...
          if (p < 1) {
            ...
            if (meteor.correct) {
              ...
              const correctAnswer = lingkaranQuizQuestions[currentQ].options[lingkaranQuizQuestions[currentQ].correctIndex];
              setFeedback({ type: "wrong", answer: correctAnswer });
            }

            setTimeout(() => {
              setLaser(null);
              if (currentQ + 1 < lingkaranQuizQuestions.length) {
                setCurrentQ((q) => {
                  const next = q + 1;
                  setupMeteors(next);
                  return next;
    ...
    return (
      <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center animate-slide-up">
          ...
          <h2 className="font-display text-2xl md:text-3xl font-bold text-accent text-glow-accent mb-6">QUIZ LINGKARAN</h2>
  ...
  if (finished) {
    const maxScore = lingkaranQuizQuestions.length * 10;
  ...
  const q = lingkaranQuizQuestions[currentQ];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex flex-col items-center px-4 py-3">
        <div className="font-display text-xs text-muted-foreground mb-1">SOAL {currentQ + 1}/{lingkaranQuizQuestions.length}</div>
        <div className="font-display text-xl md:text-2xl font-bold text-accent text-glow-accent">SKOR: {score}</div>
      </div>
        ...
        {meteors.map((m) => (
          ...
          >
            <div className="relative">
              ...
              <span className="absolute inset-0 flex items-center justify-center font-display text-[9px] md:text-[10px] font-bold text-foreground drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] px-1 text-center leading-tight">
                {m.label}
              </span>
            </div>
            {m.hit && (
...
export default LingkaranQuizPage;
